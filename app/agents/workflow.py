...
def router_node(state: TMState):
    ...
    if intent == "chitchat":
        ...
        return END  # 또는 별도의 chitchat_node로 연결 가능

    if state.get("cache_hit"):
        logger.info("[Router] Cache Hit! Skipping Extract.")
        return "strategy_gen"

    logger.info("[Router] Routing to Insight Extract")
    return "insight_extract"
