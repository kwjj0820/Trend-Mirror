...
def yt_videos_stats(video_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    for i in range(0, len(video_ids), 50):
        ...
        params = {"key": YOUTUBE_API_KEY, "part": "statistics", "id": chunk}
...
def days_since(published_at: str) -> float:
    if not published_at:
        return 30.0
    dt_ = datetime.datetime.fromisoformat(published_at.replace("Z","+00:00"))
    delta = datetime.datetime.now(datetime.timezone.utc) - dt_
...
def collect_youtube_trend_candidates(days=30, per_query=50, pages=3) -> List[Dict[str, Any]]:
    queries = [
        "요즘 유행 음식","요즘 핫한 음식","신상 먹방","유행 음식 추천",
        "핫플 음식","편의점 신상 먹방","요즘 유행 디저트",
    ]
